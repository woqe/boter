...
#define TELEGRAM_TOKEN "1235773466:AAEK3rSEambHUYD3ciZ324gpTL79KUBy04Y"
#define CHAT_ID -279165413
#define ADMIN_USERNAME "AVGmsk"
#define ADMIN_CHAT_ID 214430860
#define AUCTION_TIME_MIN 1
    ...
    while(1){
        ...
        if(auc != nullptr){
            ...
            if(get_msk_time() >= lot.end_date){
                ...
                bot.sendMessage(to_string(CHAT_ID), "Аукцион по лоту " + markdown_free(lot.name) + " закрыт\\. Победитель @" + lot.winner_1 + " забрал лот за *" + to_string(lot.price) + "₽*\n\nПобедитель, свяжись с @hempled чтобы забрать свой лот");
                bot.sendMessage(to_string(ADMIN_CHAT_ID), "Аукцион по лоту " + markdown_free(lot.name) + " закрыт \\(id: " + to_string(lot.id) + "\\)\\\n1 место: @" + lot.winner_1 + ", стоимость: " + to_string(lot.winner_1_summ) + "\n" + "2 место: @" + lot.winner_2 + ", стоимость: " + to_string(lot.winner_2_summ) + "\n"+ "3 место: @" + lot.winner_3 + ", стоимость: " + to_string(lot.winner_3_summ));
                    ...
                    switch(update.get_callback_type()) {
                        case Telegram::Update::CALLBACK_TYPE::NEW_BET:{
                            if(!user.get_username().size()){
                                ...
                                int message = user.chat_sendMessage(bot, markdown_free(update.get_callback_query().get_message().get_from().get_first_name()) + ", установи юзернейм \\(ник пользователя\\) в настройках аккаунта, прежде чем участвовать в аукционе");
